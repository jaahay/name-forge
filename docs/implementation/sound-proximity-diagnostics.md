...
Segment-bearing details use `SoundSegmentId`, the current engine's closed starter-inventory vocabulary. This is intentional because every modeled `SegmentSequence` currently uses that inventory.

Supporting provider-defined or dynamically registered segment identifiers requires a separately designed segment-registry contract. This diagnostic contract does not imply that extension.
