...
The active architecture sequence is tracked in [`../current-product-scope.md`](../current-product-scope.md) and parent checkpoint #198. Review #199 concluded that the engine/interface foundation is **not yet settled** and opened blockers #201, #202, and #203. Family/place semantic callbacks are not currently earned by the existing supporting-component implementation. Preserve this document as implementation history; do not treat its old slice order as the current roadmap.
