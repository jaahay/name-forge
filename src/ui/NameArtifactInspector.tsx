...
function sanitizeSpellingLimit(value: number, availableCount: number): number {
  if (!Number.isFinite(value)) return Math.min(defaultSameSoundSpellingLimit, Math.max(1, availableCount));
  return Math.max(1, Math.min(Math.max(1, availableCount), Math.floor(value)));
}
...
export function NameArtifactInspector({ artifact, eyebrow = 'Inspect', extraActions, extraSections }: NameArtifactInspectorProps) {
  ...
  const [spellingLimit, setSpellingLimit] = useState(() => sanitizeSpellingLimit(defaultSameSoundSpellingLimit, sameSoundSpellings.length));
  const visibleSameSoundSpellings = sameSoundSpellings.slice(0, sanitizeSpellingLimit(spellingLimit, sameSoundSpellings.length));
        ...
        <details className="detail-block artifact-detail-block">
          ...
          <label>
            ...
            <input
              type="number"
              min="1"
              max={Math.max(1, sameSoundSpellings.length)}
              value={spellingLimit}
              onChange={(event) => setSpellingLimit(sanitizeSpellingLimit(Number(event.target.value), sameSoundSpellings.length))}
            />
          </label>
          <p className="section-note">The engine ranks the complete supported pool before this display cap is applied. Preference rank favors conventional and profile-appropriate spellings; it is not a quality percentage.</p>
