...
export interface SpellingRankingOptions {
...
export interface RankedSpellingCandidate extends SpellingCandidate {
...
function getRule(segmentId: SoundSegmentId, text: string): SpellingRule {
  return getRules(segmentId).find((rule) => rule.text.toLowerCase() === text.toLowerCase()) ?? getRules(segmentId)[0];
}
...
function scoreCandidate(candidate: SpellingCandidate, profile: SoundProfile): number {
  return candidate.mappings.reduce((score, mapping) => score + scoreRule(getRule(mapping.segmentId, mapping.text), profile), 0);
}
