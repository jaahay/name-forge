...
export interface ExportedSelectedSpelling { id: string; text: string; rank: number; score: number; soundCandidateId: string; sequenceId: string; }
export interface ExportedSound { profileId: string; candidateId: string; sequenceId: string; transcription: string; selectedSpelling: ExportedSelectedSpelling; }
...
function exportSound(name: GeneratedName): ExportedSound { return { profileId: name.soundProfile.id, candidateId: name.sound.id, sequenceId: name.sound.sequence.id, transcription: name.sound.transcription, selectedSpelling: { id: name.spelling.id, text: name.spelling.text, rank: name.spelling.rank, score: name.spelling.score, soundCandidateId: name.spelling.soundCandidateId, sequenceId: name.spelling.sequenceId } }; }
...
export function serializeCastAsMarkdown(ensemble: GeneratedEnsemble): string {
  ...
  ensemble.names.forEach((name, index) => {
    ...
    lines.push('## ' + (index + 1) + '. ' + exported.name, '', '- Role: ' + (exported.role ?? 'Unassigned'), '- Role influence: ' + roleInfluenceText, '- Overall fit: ' + exported.score.toFixed(2), '- Format: ' + exported.format, '- Parts: ' + partText, '- Sound: ' + exported.sound.transcription + ' via `' + exported.sound.profileId + '`', '- Selected spelling: ' + exported.sound.selectedSpelling.text + ' (rank ' + exported.sound.selectedSpelling.rank + ', score ' + exported.sound.selectedSpelling.score.toFixed(2) + ')', '- Silhouette: ' + silhouetteSummary(name.silhouette), '- Readability notes: ' + diagnosticText(exported.readabilityDiagnostics), '- Variants: ' + variantText(exported.variants), '- Warnings: ' + (exported.warnings.length > 0 ? exported.warnings.join(', ') : 'none'), '');
  });
  return lines.join('\n').trimEnd() + '\n';
}
