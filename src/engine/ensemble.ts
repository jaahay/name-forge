...
export function generateEnsemble(settings: GenerationSettings, registry: SourceRegistry, lockedSlots?: LockedNameSlot[]): GeneratedEnsemble {
  ...
  return { settings: safeSettings, sourcePack: { id: pack.id, label: pack.label, description: pack.description, source: pack.source }, names: selected, diagnostics: diagnosticsFor(selected, castSize) };
}
