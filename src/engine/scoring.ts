...
function weightsForSettings(settings?: GenerationSettings): Record<ScoreKey, number> {
  if (!settings) return scoreWeights;
  return {
    pronounceability: lerp(0.08, 0.26, settings.pronounceability),
    memorability: lerp(0.06, 0.24, settings.memorability),
    novelty: lerp(0.05, 0.18, settings.novelty),
    culturalAnchoring: lerp(0.05, 0.18, settings.culturalAnchoring),
    orthographicNaturalness: lerp(0.2, 0.07, settings.orthographicWeirdness),
    styleFit: lerp(0.08, 0.14, settings.culturalAnchoring),
    silhouetteFit: 0.08,
    ensembleFit: 0.06,
  };
}
...
export function combineOverallFit(scores: Pick<NameScores, ScoreKey>, settings?: GenerationSettings): number {
  const weights = weightsForSettings(settings);
